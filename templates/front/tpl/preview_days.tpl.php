<? foreach ($this->data["days"] as $day) { ?>
    <li class="item">
        ...
        <div class="stat">
            <span class="view"><a href="<?= $day["link"] ?>#action-container"><i
                            class="icon"></i><?= $day["count"] ?></a></span>
            <? if ($day["liked"]) { ?>
                <span class="like liked" title="Liked"><i class="icon"></i><?= $day["like"] ?></span>
            <? } else { ?>
                <span class="like" title="Like"><a class="like-day" href="javascript:void(0)"
                                                   data-id="<?= $day["id"] ?>" data-like="<?= $day["like"] ?>"><i
                                class="icon"></i><?= $day["like"] ?></a></span>
            <? } ?>
            <?= $day["state"] ?>
        </div>
    </li>
