...
class Admin extends Model
{
    ...
    public function testTable()
    {
        ...
        $first = $day->where(["id" => ">0", "title" => "!"])->where(["year" => ">0"])->groupBy(["year"])
            ->groupBy(["month", "day"])->orderBy(["year" => "ASC"])->orderBy(["month" => "DESC",
                "day" => "ASC"])
            ->get();
        //      var_dump($day->count());
    }
    ...
    {
        $result = $this->db->table("#tbl_day d")->select(["#DISTINCT YEAR(d.date) AS dyear",
            "#MONTH(d.date) AS dmonth"])
            ->get()->toArray();
        $count = 0;
        $size = 0;
    ...
    public function updateDay($day)
    {
        ...
        $day["preview"] = $this->escapeString($day["preview"]);
